    ...
    >
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={`h-6 w-1.5 rounded-full bg-gradient-to-b ${item.gradient}`} />
          <p className="text-[13px] font-semibold text-foreground/90">{item.name}</p>
        </div>
        <div className="flex items-center gap-2">
          <HealthBar
            segments={[
          ...
          />
          <span className="text-[11px] text-muted-foreground/60 font-medium">{item.healthLabel}</span>
        </div>
      </div>
