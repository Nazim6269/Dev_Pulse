...
function transformPullRequestReview(
  dto: GithubPullRequestReviewDto,
): GithubPullRequestReview {
  return {
    id: String(dto.id),
    reviewer: dto.user.login,
    reviewerAvatarUrl: dto.user.avatar_url,
    state: dto.state,
    submittedAt: dto.submitted_at,
  };
}

function transformPullRequestFile(
  dto: GithubPullRequestFileDto,
): GithubPullRequestFile {
  return {
    sha: dto.sha,
...
export class GithubService {
  ...
  async getPullRequestReviews(
    ...
    try {
      const { data } = await this.repo.getPullRequestReviews<
        GithubPullRequestReviewDto[]
      >(username, repo, pullNumber);
      return data.map(transformPullRequestReview);
    } catch (error) {
      logger.error(
        String(error),
        `Error fetching PR reviews for ${username}/${repo}#${pullNumber}`,
      );
      throw parseError(error);
    }
  }
  ...
  async getPullRequestFiles(
    ...
    try {
      const { data } = await this.repo.getPullRequestFiles<
        GithubPullRequestFileDto[]
      >(username, repo, pullNumber);
      return data.map(transformPullRequestFile);
    } catch (error) {
      logger.error(
        String(error),
        `Error fetching PR files for ${username}/${repo}#${pullNumber}`,
      );
      throw parseError(error);
    }
  }
