...
import {
  ...
  profileConstraints,
} from "@/pages/dashboard/settings/data/settings.data";
...
export function useProfileSettings() {
  const [form, setForm] = usePersistentSettingsState<ProfileSettingsForm>(
    storageKey,
    defaultProfileSettings,
  );
  ...
  const updateField = useCallback(
    <TField extends keyof ProfileSettingsForm>(
      field: TField,
      value: ProfileSettingsForm[TField],
    ) => {
      setHasSaved(false);
