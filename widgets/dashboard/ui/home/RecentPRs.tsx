    ...
    <div className="bg-card border border-border rounded-2xl p-5 flex flex-col gap-4 shadow-sm h-110">
      ...
      <div className="flex flex-col divide-y divide-border-muted">
        {prs.map((pr, i) => (
          <div
            key={i}
            className="py-3 flex items-start gap-3 group cursor-default"
          >
            <div
              className={`mt-0.5 w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${
                pr.state === "merged" ? "bg-violet-500/15" : "bg-amber-500/15"
              }`}
            >
              {pr.state === "merged" ? (
            ...
            <span
              className={`shrink-0 text-[10px] px-1.5 py-0.5 rounded-full border mt-0.5 ${
                pr.state === "merged"
