...
import { PullRequestHeader } from "@/widgets/dashboard/ui/pull-request/PullRequestHeader";
import { PullRequestList } from "@/widgets/dashboard/ui/pull-request/PullRequestList";
import { StatusBadge } from "@/widgets/dashboard/ui/pull-request/StatusBadge";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { usePullRequests } from "@/widgets/dashboard/model/pull-request/usePullRequests";
...
function SummaryCard({
  ...
  const toneClass = {
    violet:
      "border-violet-500/20 bg-violet-500/10 text-violet-600 dark:text-violet-200",
    amber:
      "border-amber-500/20 bg-amber-500/10 text-amber-600 dark:text-amber-200",
    slate:
      "border-slate-500/20 bg-slate-500/10 text-slate-600 dark:text-slate-200",
    emerald:
  ...
  return (
    <Card className="rounded-2xl border-border bg-card shadow-sm transition-all duration-200">
      <CardContent className="p-5">
        ...
        <p className="mt-4 text-3xl font-semibold text-foreground tracking-tight">
          {value}
        </p>
      </CardContent>
    </Card>
